...
<%@ attribute name="listItems" required="true" type="java.util.List<java.lang.String>" rtexprvalue="true"
              deferredValue="false" %>
...
<div class="${c}">
    ...
    <div data-tooltip aria-haspopup="true" class="errors has-tip tip-right radius" title='${errors}'></c:if>
        <sf:select path="${path}" id="${id}" placeholder="${placeholder}"
                   cssStyle="${cssStyle}" cssClass="control ${cssClass}">
            <sf:option value="NONE" label="--- انتخاب کنید ---"/>
            <sf:options items="${listItems}"/>
        </sf:select>
        <c:if test="${not empty iconRight}">
